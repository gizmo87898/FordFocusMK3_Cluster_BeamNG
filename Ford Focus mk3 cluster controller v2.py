...
hdc = False # not recved from game
tpms = False #tbd
cruise_control = False # tbd
cruise_control_speed = 80 # tbd
foglight = True #works, not recved from game
parking_lights = False #doesnt work, also not revced from game
...
while True:
    ...
    if elapsed_time_100ms >= 0.05:
        messages_100ms = [
            ...
            can.Message(arbitration_id=0x40, data=[ # Airbag light, odometer increment
                0xFF, random.randint(0,255), 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF], is_extended_id=False),
            
            can.Message(arbitration_id=0x60, data=[ # Seatbelt
                seatbelt*96,0,0,0,0,0,0,0], is_extended_id=False),
            ...
            can.Message(arbitration_id=0x240, data=[ # handbrake
            ...
            can.Message(arbitration_id=0x320, data=[ #Fuel 
                0xe, 0xe, 0x1c, 2, 0xe, 0xe, 0xe, 0xe], is_extended_id=False),
            
            can.Message(arbitration_id=0x360, data=[ # Engine Temp
                0x00, 0x00, 0x00, 0x44, 0x80, 0xFE, 0x22, 0x22], is_extended_id=False),
    ...
    if elapsed_time_5s >= 3:
        ...
        driver_door = not driver_door
        passenger_door = not passenger_door
        driver_rear_door = not driver_rear_door
        passenger_rear_door = not passenger_rear_door
        hood = not hood
        trunk = not trunk
        airbag = not airbag
